...
dframe1<-read.table("Data/SamplesNoctThreshold.txt", header=TRUE)
...
### Let's first look at pollen load per-sample
...
chkres(model1P, dframe1$Treatment, dframe1$Season)  # surprisingly despite the overdispersion these residuals look ok-ish, though not perfect
...
### Let's now look at pollen types per-sample
### Plot it against treatment so you have an idea of what to expect
...
# possibly slight overdispersion
...
# try a neg binom instead

model2NB <- glmer.nb(PollenTypes ~ Treatment * Season + Treatment * Order
                     + (1|Site),
                     data = dframe1)

chkconv(model2NB)
chkres(model2NB, dframe1$Treatment, dframe1$Season)  # residuals probably slightly better than Poisson

summary(model2NB)
drop1(model2NB, test="Chi")


# trim out interactions

model2NBa <- glmer.nb(PollenTypes ~ Treatment + Season + Order
                     + (1|Site),
                     data = dframe1)

chkconv(model2NBa)
chkres(model2NBa, dframe1$Treatment, dframe1$Season)  # residuals probably slightly better than Poisson

summary(model2NBa)
drop1(model2NBa, test="Chi")
